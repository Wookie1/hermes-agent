...
def _maybe_auto_subscribe(conn: Any, task_id: str) -> bool:
    ...
    try:
        ...
        # *creating* session's profile (e.g. an orchestrator profile that
        # has no gateway of its own), and the running gateway skips them.
        notifier_profile = None
        try:
            notifier_profile = cfg_get(cfg, "kanban", "notifier_profile", default=None) if cfg else None
        except Exception:
            notifier_profile = None
        notifier_profile = notifier_profile or (
